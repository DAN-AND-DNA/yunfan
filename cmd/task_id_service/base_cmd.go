...
var (
	...
	base_cmd        = &cobra.Command{
		...
		RunE: func(_ *cobra.Command, _ []string) error {
			...
			if len(ptr_env_runtime.Etcd_addrs) == 0 {
				return errors.New("need etcd addr")
			}
			...
			if ptr_env_runtime.Arpc_swag != "" {
				...
				case "off":
				default:
					return errors.New("bad arpc swag type")
				}
			}
...
type Env_runtime struct {
	...
	Etcd_addrs            []string `json:"etcd_addrs"`
...
func Execute() {
	...
	base_cmd.Flags().IntVarP(&(ptr_env_runtime.Arpc_conn_idle), "arpc-conn-idle", "", 20, "arpc连接保持时间")
	base_cmd.Flags().StringVarP(&(ptr_env_runtime.Arpc_swag), "arpc-swag", "", "off", "是否开启arpc的swag页面：[on | off]")
	base_cmd.Flags().StringSliceVarP(&(ptr_env_runtime.Etcd_addrs), "etcd-addrs", "", []string{}, "etcd服务地址")
	base_cmd.SetHelpFunc(func(*cobra.Command, []string) {
		fmt.Println(`
任务id服务

Usage:
  task_id_service [flags]

Flags:
      --arpc-app-protocol string            arpc应用层协议类型：[gob | json] (default "json")
      --arpc-conn-idle int                  arpc连接保持时间 (default 20)
      --arpc-listen-port string             arpc监听端口
      --arpc-network-protocol string        arpc网络层协议类型：[tcp | udp] (default "tcp")
      --arpc-swag string                    是否开启arpc的swag页面：[on | off] (default "off")
